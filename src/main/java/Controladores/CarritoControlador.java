...
public class CarritoControlador extends HttpServlet{
            ...
            throws ServletException, IOException 
    {
        ...
        CarritoVO caVO = new CarritoVO(id, usuarioId);
 
        CarritoDAO caDAO = new CarritoDAO(caVO);

        
        switch(opcion)
        {
            case 1:
                if(caDAO.agregarRegistro())
                {
                    request.setAttribute("mensajeExito", "Carrito registrado correctamente");
                    request.getRequestDispatcher("ConsultarProductoPedido.jsp").forward(request, response);
                }
                else
                {
                    request.setAttribute("mensajeError", "Carrito no registrado correctamente");
                }
                request.getRequestDispatcher("RealizarPedido.jsp").forward(request, response);
            break;
            case 2:
                if(caDAO.eliminarRegistro())
                {
                    request.setAttribute("mensajeExito", "Carrito eliminado correctamente");
                }
                else
                {
                    request.setAttribute("mensajeError", "Carrito no eliminado correctamente");   
                }
